...
resource "aws_iam_role" "fullstack-bank_role" {
  name = "fullstack-bank-role"
...
resource "aws_iam_role_policy_attachment" "fullstack-bank_role_attach" {
  role       = aws_iam_role.fullstack-bank_role.name
  policy_arn = "arn:aws:iam::aws:policy/AdministratorAccess"
}

# Create an Instance Profile and Attach the Role
resource "aws_iam_instance_profile" "fullstack-bank_instance_profile" {
  name = "fullstack-bank-instance-profile"
  role = aws_iam_role.fullstack-bank_role.name
}
...
resource "aws_instance" "web" {
  ...
  iam_instance_profile = aws_iam_instance_profile.fullstack-bank_instance_profile.name

  tags = {
    Name = "fullstack-bank clone"
  }
  root_block_device {
    volume_size = 30
  }
